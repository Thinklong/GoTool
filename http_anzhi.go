...
var ProxyApi = "http://thinklong.sinaapp.com/proxy.php?act=get"
...
func main() {
	...
	for key, _ := range ProxyData {
		...
		proxy_array[key] = string(proxy_list.Url)
...
func del_proxy(proxy_url string) {
	url := ProxyDelApi + proxy_url
	fmt.Println(url)
	httpx := Newhttppool(url, "", "GET", "", 10)
	httpx.http_send(0)
}
...
func (h *http_pool) http_send(is_content int) (html string) {
	...
	if err != nil {
		...
		fmt.Println("请求失败！")
		html = "false"
		del_proxy(h.ProxyUrl)
		fmt.Println(proxy_list.Id)
		//resq.Body.Close()
		return
	}
